...
#### 头部搜索组件 search
```
  当点search组件的，发布博客的图片，如果已授权，会跳转到发布博客页面，如果没有授权，会弹出授权组件login，授权成功后跳转到发布博客页面
  发布博客页面 blog-edit, 涉及到 图片选择以及上传到云服务器的逻辑
```
#### 博客卡片组件 blog-card
```
博客卡片组件中又包含 - ’评论，分享组件‘ blog-ctrl:
blog-ctrl组件的逻辑: 如果没有授权，弹出授权组件，成功授权后弹出评论框，如果之前已经授权则直接弹出评论框

博客卡片会复用到评价页面blog-comment:点击博客卡片组件，会跳转到博客评价页面 blog-comment,blog-comment页面由3部分构成，顶部的博客卡片组件blog-card,中间的评论集合，底部的blog-ctrl组件，此时点击评论按钮，会弹出评论框，成功评论后会刷新页面，把最新的一条评论显示在最上面
```
